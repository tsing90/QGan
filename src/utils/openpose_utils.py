...
def create_label(shape, joint_list, person_to_joint_assoc):
    ...
    for limb_type in range(17):  # NUM_LIMBS - 2 = 17
...
def get_pose(img_orig, param, heatmaps, pafs):
    ...
    #print (joint_list_per_joint_type)  # test only
...
def get_pose_coord(img_orig, param, heatmaps, pafs):
    shape = img_orig.shape[:2]
    # Bottom-up approach:
    # Step 1: find all joints in the image (organized by joint type: [0]=nose,
    # [1]=neck...)
    ...
    return label, joint_list_per_joint_type
    
def get_pose_new(img_orig, param, heatmaps, pafs, new_joint_list_per_joint_type):
    shape = img_orig.shape[:2]
    # Bottom-up approach:
    # Step 1: find all joints in the image (organized by joint type: [0]=nose,
    # [1]=neck...)
    ...
    joint_list = np.array([tuple(peak) + (joint_type,) for joint_type,
                                                           joint_peaks in enumerate(new_joint_list_per_joint_type) for peak
                           in joint_peaks])
